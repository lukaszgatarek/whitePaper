---
title: 'Methodology behind the statistical engine of Trendposer platform. Cryptocurrency trend analysis, portfolio construction and hedging with advanced econometrics'
...
abstract: "Trendposer creates statistical and technological infrastructure necessary for effective investing in cryptocurrencies. Trendposer follows a purely scientific approach combining advanced econometrics methods with up-to-date theory of financial mathematics and data processing technology developed by the team of Trendposer. The team consists of professors and PhDs in econometrics, information technology (IT), machine learning and optimization. The technology that we develop is based on a selection of papers recently published by members of our team and co-authors. The main building blocks of the statistical engine driving behind the platform stem from [@vdb:13], [@gj:17], [@gvdha:16] and [@agh:17]. This list is definitely not exhaustive. We explore years of academic and professional experience of Trendposer team members. Due to combinations of state-of-the-art techniques from multiple disciplines of the contemporary econometrics, the Trendposer platform constitutes a coherent and complete investment analysis tool for cryptocurrency market. It covers entire chain of investment decision making process: market price trend analysis, signal processing, portfolio construction, risk analysis. The most distinctive feature of this project is its innovative character in terms of combining two spuriously opponent fields of econometrics into one coherent mechanism. Bringing together classical, large sample based statistical inference, and implementational beauty of Bayesian simulation based approach, positions this project as one of the most innovative project on the ledger of modern econometric inference. It allows for building a solid platform which covers plethora of modern analytical techniques to deliver the best possible insight around portfolio analysis in cryptocurrency market. The main components of the platform are: **trend signalling tool**, **portfolio constrution tool** and **market making tool**. The trend signalling tool identifies the current trend in the market (upward or downward trend) for each cryptocurrency, so that the investor can decide on the direction of market exposure, long or short, in the underlying cryptocurrency. Portfolio construction tool is designed to hedge the investment in the trending cryptocurrency with oppsite exposures in other cryptocurrencies, statistically related to the underlying to provide the investor with variance minimizing portfolio and reduce the market risk. Finally, the market making tool is though for investors, which have ambition to trade at high frequency. The accurate statistical algorithms explore the relation of illiquid cryptocurrecies to more liquid ones to determine the accurate market price."
...
## Trend signalling tool backtesting results

The mission of the trend signaller is to identify the current market trend from the blockchain data. To test the accuracy of the model we have performed extensive statistical backtesting of the model. For each frequency at which we estimate the model, we have performed a following simulation study.



1. The sample lenght for the time series has been fixed at $400$ observation. 

For each data frequency it corresponds to another time period. For 15-minute interval this might be only $100$ hours. For daily price frequency it corrsponds to more than a year of observations. 

2. We randomly select the start period for the time series over the available data. 

For instance let us assume that for $4$-hour frequency, the starting point has been selected at 21/06/2017 2 p.m. CET. Together with the dedicated sample lenght, $400$ observations, the starting point automatically identifies the analyzed period.

3. The probability of price increase/decrease is estimated for the selected time series based on particle filtering techniques. 

Our statistical algorithms shows the probability of price increase of a given cryptocurrency at a given point of time. It is filtered in real time as the data come in the sample. \autoref{probabilityFiltered} presents an example of probability of upward price movement filtered at $4$-hour price frequency. 

![Example of upward price move probability filtering \label{probabilityFiltered}](figures/probability.png)

For example, on the $8$ th of May $2017$ at $2$ a.m. CET, the probability of price increase has been filtered at $70\%$, which means that a price increase has been more probably than a decrease at that time. To be specific, the increase is $133\%$ more probable as $0.7/0.3$ gives the result of $2.33$. We should bear in mind, that although for expository reasons the filtered probability is presented as a time series, to estimate the probability on a given timepoint only the past data has been used. 
Automatically, knowing the probability of price increase, the probability of price decrease might be also                calculated.

4. We estimate the derivative of the probability curve.

Based on machine learning algorithms we have found out that there is a substantial relation between the first derivative of the probability curve that we estimate and the price trends observed in the market.

![Derivative of the probability filtered in \autoref{probabilityFiltered} \label{probabilityDerivative}](figures/probabilityDerivative.png)

In \autoref{probabilityDerivative} we present the evolution of the probability derivative over the analyzed time period. The graphs shows how the probability changes in time

2.2 The positive probability curve indicates that the probability increases and the cryptocurrency is in a growing trend.

2.3 The negative probability curve indicates that the probability decreases and the cryptocurrency is in a decreasing trend.

2.4. If the probability curve value is 0 it means that there is no trend to characterize the cryptocurrency as it has reached its maximum or minimum.

2.6 EMClabTrendposer came up with a separate model analyzing whether the general trend changed from increase to decrease, and vice versa. 

2.5 We send you the buy and sell signals when the blue line on the 1st derivative graphs crosses the red lines, which signifies the change in a trend of the function. 
When the blue curve doesn’t go over the horizontal red lines, it is assumed that the trend is neutral.

2.6 The decreasing trend starts when the blue line crosses bottom red line from above and ends when it crosses the same line from bottom. Decreasing trend is a signal to sell.

2.7 The increasing trend starts when the blue line crosses upper red line from the bottom and ends when it crosses the same line from above. Increasing trend is a signal to buy.

2.8 The increasing trend is marked by a green sectors, and the yellow sectors shows decreasing trends.
